#!/usr/bin/env python3
'''
SIP DoS attack shell (INVITE Flood).
'''
import sys
import time

import sipmain
from sip.attacks.dos import invite_flood as invite_flood_attack


def invite_flood():
    try:
        while True:
            choice = input("\033[37m(\033[0m\033[2;31minvite-flood\033[0m\033[37m)>\033[0m ")
            if choice == 'help' or choice == '?':
                ...
                invite_flood_attack.main(sipmain.config_file, sipmain.remote_net,
                                         sipmain.listening, sipmain.verbosity,
                                         sipmain.output_file)
            elif choice == 'back':
                sipmain.sipdos()
