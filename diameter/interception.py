#!/usr/bin/env python3
'''
Diameter Interception attack shell (Update-Location-Request).
'''
import sys
import time

import diametermain
from diameter.attacks.interception import update_location as update_location_attack


def update_location():
    try:
        while True:
            choice = input("\033[37m(\033[0m\033[2;31mulr\033[0m\033[37m)>\033[0m ")
            if choice == 'help' or choice == '?':
                ...
                update_location_attack.main(diametermain.config_file, diametermain.remote_net,
                                            diametermain.listening, diametermain.verbosity,
                                            diametermain.output_file)
            elif choice == 'back':
                diametermain.diameterinterception()
                return
            elif choice == 'exit':
